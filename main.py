...
def get_predicted_value(patient_symptoms):
    ...
    for item in patient_symptoms:
        if item in symptoms_dict:
            ...
            # If the symptom is not found, return None or raise an error
            return None
    return diseases_list[svc.predict([input_vector])[0]]
...
def home():
    if request.method == 'POST':
        ...
        if symptoms == "Symptoms":
            ...
            if invalid_symptoms:
                message = f"The following symptoms are not recognized: {', '.join(invalid_symptoms)}. Please enter valid symptoms."
                return render_template('index.html', message=message)

            predicted_disease = get_predicted_value(user_symptoms)

            if predicted_disease is None:
                message = "The symptoms entered do not match any known disease. Please try again with valid symptoms."
                return render_template('index.html', message=message)
